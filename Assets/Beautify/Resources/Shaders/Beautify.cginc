	...
	float getRandom(float2 uv) {
		return frac(sin(_Time[1] + dot(uv, float2(12.9898, 78.233)))* 43758.5453);
	}
	...
	void beautifyPass(v2f i, inout float3 rgbM) {
		...
		float4 scrLum     = tex2D(_ScreenLum, i.uv);
   	 	float4 dirt       = tex2D(_OverlayTex, i.uv);
		...
		// 9. Final contrast + brightness
  		...
  		float2 vd         = float2(i.uv.x - 0.5, (i.uv.y - 0.5) * _VignettingAspectRatio);
   		...
   		// 11. Sepia
   		#if SEPIA
   		       rgbM.r     = dot(rgbM, float3(0.393, 0.769, 0.189));
               rgbM.g     = dot(rgbM, float3(0.349, 0.686, 0.168));   
               rgbM.b     = dot(rgbM, float3(0.272, 0.534, 0.131));
        #endif
	...
	float4 fragBeautify (v2f i) : SV_TARGET {
		#if THERMAL_VISION
		i.uv.x += 0.0009 * sin(_Time[2] + i.uv.y * 20.0) / (1.0 + 10.0 * dot(i.uv - 0.5.xx, i.uv.xy - 0.5.xx));	// wave animation
		#endif
