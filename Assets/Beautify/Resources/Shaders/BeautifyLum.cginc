	...
	half4 fragLum (v2f i) : SV_TARGET {
		...
		c.rgb = max(c.rgb - _Bloom.yyy, 0);
   		return c;
   	}

   	half4 fragLumAntiflicker(v2f i) : SV_TARGET {
		half3 offsets = _MainTex_TexelSize.xyx * half3(1,1,-1);
		half4 c1 = tex2D(_MainTex, i.uv - offsets.xy);
		half4 c2 = tex2D(_MainTex, i.uv - offsets.zy);
		half4 c3 = tex2D(_MainTex, i.uv + offsets.zy);
		half4 c4 = tex2D(_MainTex, i.uv + offsets.xy);
		...
		c1.rgb = max(c1.rgb - _Bloom.yyy, 0);
   		return c1;
	}
	...
	half4 fragResample(v2f i) : SV_TARGET {
		half3 offsets = _MainTex_TexelSize.xyx * half3(1,1,-1);
		half4 c1 = tex2D(_MainTex, i.uv - offsets.xy);
		half4 c2 = tex2D(_MainTex, i.uv - offsets.zy);
		half4 c3 = tex2D(_MainTex, i.uv + offsets.zy);
